...
FileImplPtr LittleFSImpl::open(const char* path, OpenMode openMode, AccessMode accessMode) {
    ...
    if ((openMode & OM_CREATE) && strchr(path, '/')) {
        ...
        if (pathStr) {
            ...
            while (ptr) {
                *ptr = 0;
                int mdrc = lfs_mkdir(&_lfs, pathStr);
                if (mdrc == 0 && timeCallback) {
                   time_t creation = timeCallback();  // File didn't exist or otherwise, so we're going to create this time
                   if (creation) {
                      int rc2 = lfs_setattr(&_lfs, pathStr, 'c', (const void *)&creation, sizeof(creation));
                      if (rc2< 0) {
    ...
    if (rc == LFS_ERR_ISDIR) {
        ...
        DEBUGV("LittleFSDirImpl::openFile(dir): rc=%d fd=%p path=`%s` openMode=%d accessMode=%d err=%d\n",
               rc, nullptr, path, openMode, accessMode, rc);
        ...
        if (timeCallback && (flags  & LFS_O_CREAT)) {
            ...
            if (creation) {
              int rc = lfs_setattr(&_lfs, path, 'c', (const void *)&creation, sizeof(creation));
              if (rc < 0) {
                DEBUGV("Unable to set creation time on '%s' to %d\n", _name.get(), _creation);
              }
            }
...
DirImplPtr LittleFSImpl::openDir(const char *path) {
    ...
    auto dir = std::make_shared<lfs_dir_t>();
     memset(dir.get(), 0, sizeof(lfs_dir_t));
    int rc;
    const char *filter = "";
    if (!pathStr[0]) {
        // openDir("") === openDir("/")
...
int LittleFSImpl::lfs_flash_read(const struct lfs_config *c,
    ...
    return me->_spiflash->readByteArray(addr, static_cast<uint8_t*>(dst), size) ? 0 : -1;
    return 0;
}
...
int LittleFSImpl::lfs_flash_prog(const struct lfs_config *c,
    ...
    return me->_spiflash->writeByteArray(addr,static_cast<const uint8_t*>(src), size) ? 0 : -1;
}
...
int LittleFSImpl::lfs_flash_erase(const struct lfs_config *c, lfs_block_t block) {
    ...
    switch(size) {
	case 4096:
	   ret = me->_spiflash->eraseSector(addr);
   	   break;
	case 32768:
	   ret = me->_spiflash->eraseBlock32K(addr);
   	   break;
	case 65536:
	   ret = me->_spiflash->eraseBlock64K(addr);
   	   break;
	default:
  	   Serial.printf("Erase size %d\n", size);
           ret = false;
           break;
    }
    return (ret ? 0: -1); 
...
SPIFlash FLASH(17);
FS LittleFS = FS(FSImplPtr(new littlefs_impl::LittleFSImpl(&FLASH, FS_PHYS_ADDR, 0, /*FS_PHYS_SIZE,*/ FS_PHYS_PAGE, FS_PHYS_BLOCK, FS_MAX_OPEN_FILES)));
