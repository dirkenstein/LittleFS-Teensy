...
#define FS_PHYS_SIZE 1048576
#define FS_PHYS_PAGE 4096
...
{
public:
    LittleFSImpl(SPIFlash * spi, uint32_t start, uint32_t size, uint32_t pageSize, uint32_t blockSize, uint32_t maxOpenFds)
        : _spiflash(spi) , _start(start) , _size(size) , _pageSize(pageSize) , _blockSize(blockSize) , _maxOpenFds(maxOpenFds),
          _mounted(false) {
        memset(&_lfs, 0, sizeof(_lfs));
        memset(&_lfs_cfg, 0, sizeof(_lfs_cfg));
        if (_size == 0) {
          if(!_spiflash->begin()) {
              DEBUGV("Flash begin failed");
          };
        } else {
          if(!_spiflash->begin(_size)) {
              DEBUGV("Flash begin failed");
          };
        }
        _size = _size == 0 ? _spiflash->getCapacity() : _size;
        _blockSize = _blockSize == 0 ? _spiflash->getPageSize() : _blockSize; 
        Serial.printf("__blocksize %d\n", _blockSize);
    ...
    bool mkdir(const char* path) override {
        ...
        if (rc == 0 && timeCallback) {
            ...
            if (creation) {
              int rc2 = lfs_setattr(&_lfs, path, 'c', (const void *)&creation, sizeof(creation));
              if (rc2< 0) {
